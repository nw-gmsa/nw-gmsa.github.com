...
| [06 - EU Laboratory Report: FHIR Messages to a FHIR Document](https://github.com/nw-gmsa/Testing/blob/main/notebooks/06-eu-laboratory-report-fhir-document.ipynb) | Builds a FHIR Document (`Composition`-led `Bundle`) from the same ctDNA source data as 04/05 - the shape NHS England's Unified Genomic Record (UGR) Phase 2 expects. | [ctDNA NHS England Unified Genomic Record (UGR)](ctDNAUGR.html), [NE&Y Management Information (ctDNA)](overview.html#technical-detail) |
