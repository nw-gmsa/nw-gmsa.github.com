...
Description: """
Ask at Order Entry questions and digital manifest fields for a **distributed WGS (dWGS)**
sub-contracted order (IHE LTW `LAB-35`), sent by a Requesting Genomic Laboratory (RGL) to
North West Genomics acting as Sequencing Genomic Laboratory (SGL). Based on the same
approach as [GenomicTestOrder](Questionnaire-GenomicTestOrder.html), covering the 37
national digital manifest fields (NHS England `RGL to SGL SOP` Appendix 3) plus 5 local
extension fields, with `item.definition` mappings to FHIR where a confirmed mapping
exists - see [dWGS](dWGS.html#field-mapping-csv--hl7-v2--fhir).
"""
Usage:  #definition
